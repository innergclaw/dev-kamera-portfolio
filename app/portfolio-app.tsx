...
export function PortfolioApp() {
  ...
  return <main><Nav /><Hero onPlay={() => setSelected({ id: "hero", title: hero.title, synopsis: hero.synopsis, meta: hero.meta, thumbnail: "/thumbnails/hero.jpg", video: hero.video })} /><section id="library" className="library-shell" aria-label="Video library"><Row label="Continue Watching" items={library.continueWatching} variant="progress" onSelect={setSelected} /><Row label="Dev Kamera Originals" items={library.originals} variant="original" onSelect={setSelected} /><Row label="Top 10 Today" items={library.topTen} variant="rank" onSelect={setSelected} /><Row label="New Releases" items={library.newReleases} variant="new" onSelect={setSelected} /></section><section id="about" className="about-section"><p className="eyebrow">About Dev Kamera</p><h2>A working portfolio, not a highlight reel.</h2><p>Dev Kamera is a home for finished work, the decisions behind it, and the next frame in progress.</p></section><section id="work-with-me" className="work-section"><div><p className="eyebrow">Work With Me</p><h2>Have a story worth shaping?</h2></div><a className="button button-primary" href="#work-with-me">Start a conversation</a></section><footer><span>© Dev Kamera</span><span>Not affiliated with any streaming service. Just built like one.</span><div><a href="https://instagram.com/">Instagram</a><a href="https://vimeo.com/">Vimeo</a><a href="https://youtube.com/">YouTube</a></div></footer>{selected && <VideoModal item={selected} onClose={() => setSelected(null)} />}</main>;
}
